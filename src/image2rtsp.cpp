...
Image2rtsp::Image2rtsp() : Node("image2rtsp"){
    ...
    appsrc = NULL;
    // Setup the pipeline
    pipeline_tail = " key-int-max=10 ! video/x-h264, profile=baseline ! rtph264pay name=pay0 pt=96 )";
    if (camera == false){
        ...
        pipeline = "( " + source + " ! videoconvert ! videoscale ! " + caps_1 + framerate + caps_2 + " ! nvh264enc preset=low-latency bitrate=" + bitrate + pipeline_tail;
